...
def get_message_text(message: Message, reply: bool = False):
    return (
        '📷 Фото' if message.photo and reply else
        message.file.emoji + ' Стикер' if message.sticker and reply else
        '📹 Видеосообщение' if message.video_note and reply else
        '📹 Видео' if message.video and reply else
        '🖼 GIF' if message.gif and reply else
        '📊 Опрос' if message.poll else
        '📍 Местоположение' if message.geo else
        '👤 Контакт' if message.contact else
        f'🎵 Голосовое сообщение {strftime(message.voice.attributes[0].duration)}' if message.voice else
        f'🎧 Музыка {strftime(message.audio.attributes[0].duration)}  {message.audio.attributes[0].performer} - {message.audio.attributes[0].title}' if message.audio else
        f'💾 Файл {message.file.name}' if isinstance(message.media, types.MessageMediaDocument) and not get_message_media(message) else
        f'{message.media.emoticon} Дайс {message.media.value}' if isinstance(message.media, types.MessageMediaDice) else
        f'Service message {message.action.to_dict()["_"]}' if isinstance(message, types.MessageService) else ''
    )
